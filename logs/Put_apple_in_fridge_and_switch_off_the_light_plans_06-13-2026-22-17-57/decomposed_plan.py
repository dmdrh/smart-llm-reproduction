...
### Explanation:
- **put_apple_in_fridge**: This function handles the steps required to pick up the apple and place it in the fridge.
- **switch_off_light**: This function handles the steps required to go to the light switch and turn it off.
- Both functions are executed in parallel using threads, allowing for efficient task completion. After starting both threads, the main program waits for both to finish before concluding the task.