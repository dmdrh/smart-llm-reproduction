...
if __name__ == "__main__":
    ...
    for i, (plan, solution) in enumerate(zip(decomposed_plan, allocated_plan)):
        ...
        messages = [
            {"role": "system", "content": "You are a Robot Task Allocation Expert. When generating the code, decide the execution structure based on data dependencies between subtasks: subtasks that are independent of each other (where the result of one is NOT the input of another) MUST be run in parallel using threading.Thread. Subtasks that have a dependency (the output of subtask A is the input of subtask B) MUST be run sequentially, and you must synchronize dependent threads with join() before starting the dependent subtask."},
            {"role": "user",   "content": curr_prompt}
        ]
